...
async def test_open_market_simulation(
    ...
    Returns actual screener results with forced LIVE pricing.
    """
    from services.data_provider import fetch_live_stock_quote, fetch_options_chain
    from services.greeks_service import calculate_greeks, normalize_iv_fields
    
    test_symbols = ["INTC", "AMD", "BAC"]
    results = []
    ...
    for test_symbol in test_symbols:
        ...
        # Find first option with valid BID (for CC) or ASK (for PMCC long)
        sample_option = None
        for opt in (options or []):
            bid = opt.get("bid", 0) or 0
            ask = opt.get("ask", 0) or 0
            if endpoint == "covered-calls" and bid > 0:
                sample_option = opt
                break
            elif endpoint == "pmcc" and ask > 0 and bid > 0:
                sample_option = opt
                break
        ...
        if sample_option:
            results.append({
                ...
                "option": {
                    "strike": sample_option.get("strike"),
                    "expiry": sample_option.get("expiry"),
                    "dte": sample_option.get("dte"),
                    "bid": sample_option.get("bid"),
                    "ask": sample_option.get("ask"),
                    "iv": sample_option.get("implied_volatility")
                }
            })
    ...
    return {
        ...
        "note": "This simulates what the response would look like during market OPEN hours (9:30 AM - 4:00 PM ET). Empty results = no valid BID/ASK available (market closed)."
    }
